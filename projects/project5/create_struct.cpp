...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n_structure_per_process; ++i) {
        ...
        for (int j = 0; j < DOUBLE_MAX; ++j) {
            data[i].values[j] = (j < data[i].n_values ? static_cast<double>(rand()) / RAND_MAX : 0.0);
        }
    }

    // 1- Set up struct type description
    ...
    MPI_Gather(data, n_structure_per_process, custom_dt,
               gathered_data, n_structure_per_process, custom_dt,
               0, MPI_COMM_WORLD);

    // Print gathered results on root
    if (rank == 0) {
